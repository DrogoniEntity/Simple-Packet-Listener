...
{	
	...
	static {
		PLAYER_LISTENER_CLASS_NAME = NmsReflection.isRunningInDeveloppementJar() ? 
				"server.network.ServerGamePacketListenerImpl" :
				"server.network.PlayerConnection";
		
		SERVER_LISTENER_CLASS_NAME = NmsReflection.isRunningInDeveloppementJar() ?
				"server.network.ServerConnectionListener" :
				"server.network.ServerConnection";
		
		CONNECTION_CLASS_NAME = NmsReflection.isRunningInDeveloppementJar() ?
				"network.Connection" :
